...
extension CacheSystemEvents: DependencyKey {
  static let liveValue = CacheSystemEvents {
    ...
    let willResign = AsyncStream { continuation in
      Task {
        for await notification in center.notifications(named: .willResignActiveNotification) {
    ...
    let willTerminate = AsyncStream { continuation in
      Task {
        for await notification in center.notifications(named: .willTerminateNotification) {
          continuation.yield(SystemEvent.applicationWillSuspend)
        }
      }
    }
